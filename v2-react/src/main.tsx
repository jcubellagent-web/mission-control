...
function workerRouteIsFresh(worker: ActiveModelRoute) {
...
function synchronizedAgentFocus(
  manualSelection: AgentId | null,
  workingAgents: AgentId[],
): AgentId | null {
  if (workingAgents.length === 1) return workingAgents[0];
  if (workingAgents.length > 1) {
    return manualSelection && workingAgents.includes(manualSelection)
      ? manualSelection
      : workingAgents[0];
  }
      ...
      <section className="kiosk-grid">
        <section id="brain-feed" className={`brain-hero-panel brain-hero-panel--single${sectionCueClass("brain", liveCues)}`}>
          ...
          <BrainHero
            ...
            liveCues={liveCues}
            selectedAgent={synchronizedSelectedAgent}
...
function BrainAtlasEcosystemView({
  ...
  return (
    <section
      className={`brain-ecosystem-view brain-atlas-section is-unified${hasActiveControllers ? " has-active-controllers" : " is-idle"}${activeLeases.length ? " has-change-lease" : ""}${selectedAgent ? " has-trace-focus" : ""}${activeMemoryAgent ? ` has-active-memory-agent active-memory-agent-${activeMemoryAgent}` : ""}`}
      data-atlas-region={"unified"}
      data-memory-signal={signal || "idle"}
      data-active-agent-count={activeAgentIds.length}
      data-active-agents={activeAgentIds.join(",")}
      data-active-memory-agent={activeMemoryAgent || ""}
        ...
        <div className="brain-ecosystem-stage memory-flow-map" data-atlas-layer="memory">
          <div className="brain-stage-toolbar">
            ...
            <div>
              ...
              <button type="button" className="brain-selected-event" disabled={!selectedAgent && !selectedEvent} onClick={() => selectedAgent ? setDrawer({ kind: "trace", agent: selectedAgent }) : selectedEvent && setDrawer({ kind: "event", row: selectedEvent })}>
                {selectedAgent && activeAgentSet.has(selectedAgent) ? "Live work" : selectedAgent ? "Trace details" : "Selected event"} <i /> {selectedAgent ? AGENTS[selectedAgent].label : selectedEvent ? compactText(selectedEvent.workLabel, 28) : "No recent receipt"} <i /> {selectedAgent && !selectedTraceHasMemory ? "Memory quiet" : selectedEvent ? "1 receipt" : "0 receipts"} <span>›</span>
              </button>
            </div>
          </div>
          ...
          <div className={`brain-memory-core${selectedTraceHasMemory ? " has-agent-focus has-trace-memory" : selectedAgent ? " has-work-focus" : ""}`} aria-label="Governed shared memory lifecycle">
            {agentRows.map(({ agent, liveWork, controllers, workers, load }, index) => {
              ...
              const routeActive = activeAgentSet.has(agent);
              ...
              return (
                ...
                >
                  ...
                  <span className={`brain-agent-node-status dot ${statusClass(liveWork.status.status)}`} aria-hidden="true" />
                  <span className="brain-agent-node-heading">
                  ...
                  <span className={`brain-agent-node-route${routeActive ? " is-route-active-label" : ""}`} title={`${routeStateLabel} · ${workerModelLabels.length ? `${routeLabel} controller · ${workerModelLabels.join(" + ")} workers` : `${routeLabel} controller`}`} style={{ "--route-color": route.color } as React.CSSProperties}>
                    {routeActive ? routeStateLabel : selectedRow && workerModelLabels.length ? `${routeLabel} + ${workerModelLabels.join(" + ")}` : routeLabel}
                  </span>
            ...
            <svg viewBox="0 0 1000 220" preserveAspectRatio="none" aria-hidden="true" data-memory-source="governed-memory-registry">
              ...
              {memoryNodeIsActive("helpful") && memoryNodeIsActive("candidate") ? <circle className="brain-memory-packet is-feedback-packet is-live" r="4"><animateMotion dur="1.25s" repeatCount="indefinite" path="M 798 194 H 760" /></circle> : null}
              {memoryNodeIsActive("candidate") && memoryNodeIsActive("durable") ? <circle className="brain-memory-packet is-feedback-packet is-live" r="4"><animateMotion dur="1.35s" repeatCount="indefinite" path="M 620 194 H 466" /></circle> : null}
              ...
              {agentRows.map(({ agent, liveWork, workers }) => {
                ...
                const workLive = liveWork.working && isFreshActiveTimestamp(workObservedAt);
                const routeEvidenceAt = [
