...
4. Create or confirm the task with `--work-scope shared-source`, then acquire the
   exclusive lease using the exact task identity:
   `python3 scripts/control_tower_change_guard.py begin --agent <agent> --objective "<specific change>" --task-id <task-id> --work-id <work-id> --run-id <run-id>`
...
8. Publish a terminal task state only after `control_tower_change_guard.py status`
   reports `lease: null`, the exact task-bound source-closeout receipt exists, and
