...
- `scripts/gemini_agent.py` checks the Antigravity-backed Gemini and can run dashboard-safe smoke prompts without writing raw prompts or model output into Control Tower sidecars.
...
Gemini is a shared specialist model layer. JOSHeX owns Gemini brokering and approvals, Josh 2.0 can report Antigravity Gemini health, JAIMES can use Gemini only as an evaluator for non-sensitive specialist summaries, and J.A.I.N can use it for scheduled dashboard-safe digests.
