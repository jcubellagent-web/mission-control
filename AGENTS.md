...
- Keep the hard human boundary at identity, money, public commitment, and irreversible external state: passkeys, MFA enrollment/recovery/factor changes, personal-account sign-ins, wallet/Base Account signing, purchases, public posting, external account deletion, and live roster/lineup submission outside standing policy still require explicit Josh approval at the moment of action.
- Narrow agent-owned exception: routine TOTP completion for an enabled account in `config/agent-mfa-broker.json` may use `scripts/agent_mfa_broker.py` on Josh 2.0 without repeated approval. The broker must bind the account to an exact origin/path and purpose, inject directly into the verified browser, expose no seed or code, and write metadata-only private receipts. Alpaca is paper-trading and market-data access only; live brokerage activation, funding, orders, transfers, recovery, and factor changes remain human-gated.
...
- Apply `agent-skills/credential-access/SKILL.md` and `config/agent-credential-access.json` whenever any agent needs a password, API key, token, authenticated browser session, or routine MFA capability. Route the work to the owning Mac mini and use the credential through an approved host-local profile or broker; never retrieve a secret into model context, shared memory, Brain Feed, a task payload, a shell argument, clipboard, file, or log. Ordinary expected 1Password CLI authorization should be verified and accepted within two seconds. Password changes, recovery, passkey/MFA enrollment, financial signing, purchases, permission expansion, public actions, and irreversible actions remain human-gated.
