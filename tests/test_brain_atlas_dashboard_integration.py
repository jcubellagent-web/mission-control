...
class BrainAtlasDashboardIntegrationTests(unittest.TestCase):
    ...
    def test_ui_contract_is_memory_first_with_on_demand_exact_proof(self) -> None:
        ...
        self.assertIn("Only governed memory receipt paths and receipt rings move when a recent exact registry timestamp exists", component)
        self.assertIn("LIVE AGENTS + GOVERNED MEMORY", component)
        self.assertIn("EXACT EXECUTION PROOF · STATIC AUDIT PATHS", component)
        self.assertNotIn('className="memory-flow-metrics"', component)
        self.assertNotIn('className="brain-atlas-context"', component)
        self.assertNotIn('className="brain-atlas-proof-utility"', component)
        self.assertNotIn('className="brain-atlas-proof-audit"', component)
        self.assertIn("selected -> used", component)
        self.assertIn("helpful outcomes", component)
        self.assertIn("active ·", component)
        self.assertIn("supersededMemoryCount", component)
        self.assertIn("registrySourceCount", component)
        ...
        self.assertIn("data-load-tier={systemLoad.tier}", component)
        self.assertIn("data-load-score={systemLoad.score}", component)
        self.assertIn("data-agent-load-tier={agentLoad.tier}", component)
        self.assertIn("data-agent-load-score={agentLoad.score}", component)
        self.assertIn("function brainAtlasAgentLoad(", component)
        self.assertIn("function brainAtlasSystemLoad(", component)
        self.assertIn("claimedPriority >= 80 ? 3", component)
        self.assertIn("workingAgents >= 2 ? 1 : 0", component)
        self.assertIn("Moving paths represent recent governed memory receipts only", component)
        self.assertIn("RECENT REPLAY", component)
        self.assertIn("LIVE FLOW", component)
        self.assertIn("VERIFIED EVENT", component)
        self.assertIn("MEMORY_FLOW_EVENT_LIMIT = 6", component)
        self.assertIn("replayedMemoryEvents", component)
        self.assertIn("memoryFlowEventPath(activeMemoryEvent)", component)
        self.assertIn('data-memory-event={activeMemoryEvent.signal}', component)
        self.assertIn("Cross-agent relay", component)
        self.assertIn("is-review-aging", component)
        self.assertIn("is-promotion-confirmed", component)
        self.assertNotIn("setInterval", component)
        self.assertIn("brain-atlas-load-meter", component)
        ...
        self.assertIn("const safelyEllipsized = overflowX > 1", runtime_check)
        self.assertIn("style.textOverflow === 'ellipsis'", runtime_check)
        self.assertIn("Boolean(labelledContainer?.getAttribute('title')?.trim())", runtime_check)
        ...
        self.assertIn('memory-flow-worker-lane brain-agent-node-lane-data', component)
        ...
        self.assertEqual(2, component.count("memory-flow-node is-agent"))
        self.assertIn("liveWorkPresentationForAgent(agent, statusByAgent.get(agent), workItems)", component)
        self.assertIn("workingAgentCount > 0 && activity && !exactMemoryPathLive", component)
        self.assertIn("const exactMemoryPathLive = Boolean(", component)
        ...
        self.assertIn('data-operation="live-work"', component)
        self.assertIn('is-work-lifeline', component)
        ...
        self.assertIn(".memory-event-packet {", styles)
        self.assertIn(".memory-event-packet.is-amber", styles)
        self.assertIn(".memory-event-packet.is-purple", styles)
        self.assertIn(".memory-event-packet.is-green", styles)
        self.assertIn(".memory-flow-live-status", styles)
        self.assertIn(".memory-review-aging-beacon", styles)
        self.assertIn("@keyframes memory-promotion-confirmation", styles)
        self.assertIn("contain: layout paint;", styles)
        self.assertIn("@keyframes memory-node-receipt-ring", styles)
        self.assertNotIn("@keyframes memory-agent-receipt-dot", styles)
        ...
        self.assertIn('#brain-atlas[data-load-tier="light"]', styles)
        self.assertIn('#brain-atlas[data-load-tier="active"]', styles)
        self.assertIn('#brain-atlas[data-load-tier="heavy"]', styles)
        self.assertIn('.memory-flow-node.is-work-active[data-agent-load-tier="light"]', styles)
        self.assertIn('.memory-flow-node.is-work-active[data-agent-load-tier="active"]', styles)
        self.assertIn('.memory-flow-node.is-work-active[data-agent-load-tier="heavy"]', styles)
        self.assertIn("var(--atlas-agent-halo-duration, 3.4s)", styles)
        self.assertIn("var(--atlas-system-duration)", styles)
        live_map_css = styles[
        ...
        self.assertIn(".memory-flow-node-receipt-ring", styles)
        ...
        self.assertIn(".memory-flow-node.is-live .memory-flow-node-receipt-ring", reduced_motion)
        self.assertIn(".memory-event-packet", reduced_motion)
        self.assertNotIn("memory-flow-memory-receipt-dot", reduced_motion)
    ...
    def test_ecosystem_graph_bounds_icons_and_promotes_agent_state_into_graph(self) -> None:
        main = (MISSION_CONTROL / "v2-react" / "src" / "main.tsx").read_text(encoding="utf-8")
        styles = (MISSION_CONTROL / "v2-react" / "src" / "styles.css").read_text(encoding="utf-8")
        ...
        self.assertIn('data-worker-count={workers.length}', main)
        self.assertIn('data-memory-operation={agentMemorySignal?.[0] || "none"}', main)
        self.assertIn("width: 276px", styles)
        self.assertIn("min-height: 78px", styles)
        self.assertIn("border-left: 6px solid var(--agent-tone)", styles)
        self.assertIn("width: 210px; min-height: 72px", styles)
        self.assertIn("left: 56%", styles)
        self.assertIn("width: 160px", styles)
        self.assertIn("--registry-tone: #62d6c7", styles)
        self.assertIn("grid-template-rows: 38px minmax(0, 1fr)", styles)
    ...
    def test_ecosystem_graph_keeps_memory_and_work_in_one_hero_canvas(self) -> None:
        ...
        self.assertIn('className="brain-agent-worker-stack"', main)
        self.assertIn('data-parent-agent={agent}', main)
        self.assertIn('data-worker-state={workerTelemetry.state.toLowerCase()}', main)
        self.assertIn('visibleWorkers.map((worker)', main)
        self.assertNotIn("brain-agent-worker-toggle", main)
        self.assertIn(".brain-agent-worker-chip.is-active > svg", styles)
        self.assertIn(".brain-agent-worker-chip.is-returning", styles)
        self.assertIn(".brain-agent-worker-chip.is-stale", styles)
    ...
    def test_live_work_and_atlas_share_agent_focus_and_explicit_memory_windows(self) -> None:
        ...
        self.assertIn("function synchronizedAgentFocus(", main)
        self.assertIn("if (workingAgents.length === 1) return workingAgents[0];", main)
        self.assertIn("manualSelection && workingAgents.includes(manualSelection)", main)
        self.assertIn("const workingAgentIds = useMemo(() => HERO_AGENT_ORDER.filter", main)
        self.assertIn("const synchronizedSelectedAgent = synchronizedAgentFocus(selectedAgent, workingAgentIds);", main)
        self.assertEqual(main.count("selectedAgent={synchronizedSelectedAgent}"), 2)
        ...
        self.assertIn('selectedTraceHasMemory ? " has-agent-focus has-trace-memory" : selectedAgent ? " has-work-focus"', main)
        self.assertIn('selectedTraceHasMemory ? " is-trace-evidence" : ""', main)
        self.assertIn('"WORK LIVE · MEMORY QUIET"', main)
    ...
    def test_live_work_and_atlas_expose_concurrent_controller_work_without_clipping_leases(self) -> None:
        ...
        self.assertIn('className="agent-activity-stack"', main)
        self.assertIn('className="agent-rotation-counter"', main)
        self.assertNotIn('className="agent-headline-rotation"', main)
        self.assertNotIn('? "rotating" : `${controllerCount} active`', main)
    ...
    def test_brain_atlas_exposes_telemetry_backed_active_agent_routes_without_clicks(self) -> None:
        ...
        self.assertIn("const workLive = liveWork.working && isFreshActiveTimestamp(workObservedAt);", main)
        self.assertIn("const freshWorkers = workers.filter(workerRouteIsFresh);", main)
        self.assertIn("const routeLive = Boolean(workLive || freshWorkers.length > 0);", main)
        self.assertNotIn('...activeLeases.filter((lease) => lease.agent === agent).map((lease) => lease.startedAt)', main)
        self.assertIn('className={`brain-work-route-link agent-${agent} is-work-live`}', main)
        self.assertIn('data-operation="live-work"', main)
        self.assertIn('data-work-route-live="true"', main)
        self.assertIn('data-operation={memoryOperation}', main)
        self.assertIn("#brain-atlas .brain-agent-graph-node.is-route-active", styles)
        self.assertIn("#brain-atlas .brain-work-route-link.is-work-live", styles)
