...
def valid_kiosk_legibility_measurements() -> dict[str, object]:
    return {
        ...
        "todayJobs": {
            ...
            "reasonTriggerCount": 98,
            "missingReasonCount": 0,
            "objectReasonCount": 0,
            "pendingSummaryReason": "4 past occurrences are awaiting evidence. These are past occurrences, never future scheduled work. 85 occurrences are scheduled later today. They are future work, not open or overdue work.",
...
def test_layout_accepts_idle_agent_with_exact_live_memory_path() -> None:
    ...
    board_node["activity"] = "quiet"
    board_node["activityLabel"] = ""
    board_node["activityBarAnimationNames"] = []
...
def test_layout_accepts_exact_live_path_without_animation_in_reduced_motion() -> None:
    ...
    for node in atlas_nodes:
        ...
        node["memoryAnimated"] = False
        node["animated"] = False
    live_work_agents = memory["liveWorkAgents"]
    assert isinstance(live_work_agents, list)
    for card in live_work_agents:
        assert isinstance(card, dict)
        if card.get("working") is True:
            card["activityBarAnimationNames"] = ["none", "none", "none"]
