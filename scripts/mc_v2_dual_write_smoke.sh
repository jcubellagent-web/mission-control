...
"$PYTHON_BIN" "$ROOT/scripts/agent_publish.py" \
  ...
  --status info \
  --title "$TITLE" \
  --tool "mc_v2_dual_write_smoke.sh" \
  --detail "dashboard-safe v2 dual-write smoke from Josh 2.0" \
  --brain-feed \
  --v2

"$PYTHON_BIN" "$ROOT/scripts/mc_v2_verify.py" --agent joshex --expect-title "$TITLE"
