...
"$PYTHON_BIN" "$ROOT/scripts/agent_publish.py" \
  ...
  --detail "dashboard-safe JAIMES job row smoke for the current Control Tower jobs rail" \
  --brain-feed \
  --job \
  --v2
...
"$PYTHON_BIN" "$ROOT/scripts/agent_publish.py" \
  ...
  --title "$HANDOFF_TITLE" \
  --tool "mc_v2_job_handoff_smoke.sh" \
  --detail "dashboard-safe JAIMES handoff smoke for the current Control Tower approval path" \
  --handoff-to joshex \
  --brain-feed \
  --v2
